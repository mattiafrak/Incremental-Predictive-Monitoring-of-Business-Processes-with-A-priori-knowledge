...
def run_experiments(log_identificator, formula_type, rnn_type):
    ...
    if rnn_type == "CF":
        ...
        path_to_model_file = path_to_model_file_cfrt
    ...
    lines, \
        ...
        target_char_indices_time = prepare_testing_data(eventlog)
    ...
    with open('output_files/final_experiments/results/baseline/%s_%s.csv' % (eventlog[:-4], rnn_type), 'wb') as csvfile:
        ...
        spamwriter.writerow(["Prefix length", "Ground truth", "Predicted", "Levenshtein", "Damerau", "Jaccard",
                             ...
                             "Predicted Group", "Levenshtein Group", "Ground Truth Elapsed Time", "Predicted Elapsed Time", "Levenshtein Elapsed Time"])
        for prefix_size in range(prefix_size_pred_from, prefix_size_pred_to):
            ...
            for line, line_id, line_group, line_time, times, times2, times3, times4 in izip(lines_s,
                ...
                #cropped_line_time = ''.join(list(map(lambda x: str(get_int_from_unicode(x)),line_time[:prefix_size])))
                cropped_times = times[:prefix_size]
                #cropped_times_string = ' '.join(list(map(lambda x: str(x), times[:prefix_size])))
                cropped_times3 = times3[:prefix_size]
                cropped_times4 = times4[:prefix_size]
                if len(times2) < prefix_size:
                    continue  # make no prediction for this case, since this case has ended already
                #ground_truth = ' '.join(list(map(lambda x: str(get_int_from_unicode(x)) , line[prefix_size:prefix_size+predict_size])))
                ...
                #ground_truth_time = ''.join(list(map(lambda x: str(get_int_from_unicode(x)), line_time[prefix_size:prefix_size + predict_size])))
                ...
                for i in range(predict_size):
                    ...
                    #prediction_elapsed_time = get_int_from_unicode(get_symbol_time(y_time))
                    cropped_line += prediction
                    cropped_line_group += prediction_group
                    cropped_line_time += prediction_elapsed_time
                    #cropped_line_time += str(prediction_elapsed_time)
                    ...
                    #predicted_elapsed_time += str(prediction_elapsed_time)
                ...
                if len(ground_truth) > 0:
                    ...
                    #ground_truth_time = get_int_from_unicode(ground_truth_time)
                    #predicted_elapsed_time = get_int_from_unicode(predicted_elapsed_time)

                    #output.append(metrics.mean_absolute_error([ground_truth_time], [predicted_elapsed_time]))
                    #predicted_elapsed_time=unicode(predicted_elapsed_time).encode("utf-8")
                    #ground_truth_time=unicode(ground_truth_time).encode("utf-8")
