...
def train_with_data_and_time():
    ...
    for row in spamreader:
        ...
        if (int(int(row[4]) / diff)) <= r:
            line_time += get_unicode_from_int(int(int(row[4]) / diff))
        else:
            line_time += get_unicode_from_int(r)
        #line_time += get_unicode_from_int(row[4])
    ...
    fold1_time = lines_time[:elems_per_fold]
    fold1_t = timeseqs[:elems_per_fold]
    fold1_t2 = timeseqs2[:elems_per_fold]

    fold2 = lines[elems_per_fold:2*elems_per_fold]
    fold2_group = lines_group[elems_per_fold:2*elems_per_fold]
    fold2_time = lines_time[elems_per_fold:2*elems_per_fold]
    ...
    # train a 4-layer LSTM with one shared layer
    # the shared layer
    l1 = LSTM(100, consume_less='gpu', init='glorot_uniform', return_sequences=False, dropout_W=0.2)(main_input)
    #b1 = BatchNormalization()(l1)

    # the layer specialized in activity prediction
    #l2_1 = LSTM(100, consume_less='gpu', init='glorot_uniform', return_sequences=False, dropout_W=0.2)(l1)
    ...
    #l2_3 = LSTM(100, consume_less='gpu', init='glorot_uniform', return_sequences=False, dropout_W=0.2)(l1)
    #b2_3 = BatchNormalization()(l2_3)

    # the layer specialized in elapsed time prediction
    #l2_4 = LSTM(100, consume_less='gpu', init='glorot_uniform', return_sequences=False, dropout_W=0.2)(l1)
    #b2_4 = BatchNormalization()(l2_4)

    act_output = Dense(len(target_chars), activation='softmax', init='glorot_uniform', name='act_output')(l1)
    group_output = Dense(len(target_chars_group), activation='softmax', init='glorot_uniform',
                         name='group_output')(l1)
    elapsed_time_output = Dense(len(target_chars_time), activation='softmax', init='glorot_uniform',
                         name='elapsed_time_output')(l1)
    time_output = Dense(1, init='glorot_uniform', name='time_output')(l1)

    model = Model(input=[main_input], output=[act_output, group_output, elapsed_time_output, time_output])
    ...
    path_to_model = 'output_files/final_experiments/models/CFRT/' + eventlog[:-4] + \
